...
interface Payment { type: string; amount: number; amount_usd: number | null; status: string; paid_at: string | null; created_at: string }
...
export default function RevenueReport({ lang }: Props) {
  ...
  useEffect(() => {
    supabase.from("payments").select("type, amount, amount_usd, status, paid_at, created_at").then(({ data }) => {
  ...
  const stackedByType = useMemo(() => {
    ...
    const byKey = new Map(months.map(m => [m.key, m]));
    PAYMENT_TYPES.forEach(t => months.forEach(m => { m[t.value] = 0; }));
    filtered.filter(p => p.status === "success").forEach(p => {
      const d = new Date(p.paid_at ?? p.created_at);
      const m = byKey.get(`${d.getFullYear()}-${d.getMonth()}`);
      if (m) m[p.type] = (Number(m[p.type]) || 0) + usdOf(p);
    });
    return months;
  }, [filtered, lang]);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card p-5"><DollarSign className="w-5 h-5 text-green-600 mb-2" strokeWidth={2} /><div className="text-xl font-black text-green-700">{format(totalCollected)}</div><div className="text-xs text-slate mt-1">{lang === "en" ? "Total Collected" : "Total Collecté"}</div></div>
        <div className="card p-5"><Clock className="w-5 h-5 text-yellow-600 mb-2" strokeWidth={2} /><div className="text-xl font-black text-yellow-700">{format(totalPending)}</div><div className="text-xs text-slate mt-1">{lang === "en" ? "Pending" : "En attente"}</div></div>
      </div>
          ...
          <div className="card p-5">
            ...
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={stackedByType} margin={{ top: 4, right: 8, left: -16, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: "#94a3b8" }} axisLine={false} tickLine={false} />
                <Tooltip formatter={(v) => format(Number(v))} />
                <Legend wrapperStyle={{ fontSize: 11 }} formatter={(value) => { const t = PAYMENT_TYPES.find(t => t.value === value); return t ? (lang === "en" ? t.en : t.fr) : value; }} />
                {PAYMENT_TYPES.map(t => (
                  <Bar key={t.value} dataKey={t.value} stackId="rev" fill={TYPE_COLORS[t.value]} radius={[0, 0, 0, 0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </div>
