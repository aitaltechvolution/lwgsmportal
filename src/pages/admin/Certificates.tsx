...
export default function AdminCertificates() {
  ...
  const load = async () => {
    ...
    setEligible((eligRes.data ?? []) as unknown as EligibleRow[]);
  ...
  const onIssue = async (row: EligibleRow) => {
    ...
    try {
      ...
      showToast("error", lang === "en" ? "Failed to issue certificate." : "Échec de l'émission du certificat.");
    } finally {
      setIssuingKey(null);
    }
  };
    ...
    <AdminLayout title={lang === "en" ? "Certificates" : "Certificats"}>
      <div className="mb-6 animate-fade-in-up">
        ...
        <p className="text-sm text-slate mt-0.5">
          {eligible.length} {lang === "en" ? "eligible" : "éligible(s)"} · {certs.length} {lang === "en" ? "issued" : "émis"}
        </p>
      </div>
        ...
        <div className="card overflow-hidden stagger-children">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50/60 border-b border-gray-100">
                  {[lang === "en" ? "Cert #" : "N° Cert", lang === "en" ? "Student" : "Étudiant", lang === "en" ? "Program" : "Programme", lang === "en" ? "Issued" : "Émis", lang === "en" ? "Status" : "Statut", ""].map(h => (
                    <th key={h} className="text-left px-5 py-3 text-xs font-bold text-slate uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
                ...
                {filteredCerts.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-gray-50/60 transition-colors">
                      ...
                      <td className="px-5 py-3.5 font-semibold text-ink">{c.student_name}</td>
